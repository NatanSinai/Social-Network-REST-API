...
export const postSchema = metadataSchema.extend({
  title: z.string().min(3, { error: ({ minimum }) => `Title must be at least ${minimum} characters long` }),
  content: z.string().max(500, { error: ({ maximum }) => `Content must be maximum ${maximum} characters long` }),
